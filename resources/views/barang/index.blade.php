...
<div class="bg-white rounded-3xl p-8 shadow-[0_8px_30px_rgb(0,0,0,0.04)] border border-slate-100">
    <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-8 pb-4 border-b border-slate-100">
        ...
        <a href="{{ route('barang.create') }}" class="mt-4 sm:mt-0 flex items-center px-5 py-2.5 bg-gradient-to-r from-indigo-500 to-purple-600 text-white rounded-xl shadow-lg shadow-indigo-200 hover:shadow-indigo-300 transform hover:-translate-y-0.5 transition-all text-sm font-bold">
        ...
        <table class="w-full text-left border-collapse">
            ...
            <tbody class="divide-y divide-slate-100 text-sm text-slate-700 font-medium">
                ...
                <tr class="hover:bg-indigo-50/30 transition-colors group">
                    <td class="py-4 px-6 text-indigo-600 font-bold font-mono">{{ $b->kode_barang }}</td>
                    <td class="py-4 px-6">
                        <div class="flex items-center">
                            @if($b->gambar)
                                <img src="{{ asset('storage/' . $b->gambar) }}" alt="{{ $b->nama_barang }}" class="w-10 h-10 object-cover rounded-xl border border-slate-200 mr-3 shrink-0 shadow-sm">
                            @else
                            ...
                            @endif
                            <span class="font-bold text-slate-800">{{ $b->nama_barang }}</span>
                        </div>
                    </td>
                    <td class="py-4 px-6">
                        <span class="bg-slate-100 text-slate-600 py-1 px-3 rounded-full text-xs font-semibold">{{ $b->kategori->nama_kategori ?? '-' }}</span>
                    </td>
                    <td class="py-4 px-6 text-emerald-600 font-bold">Rp{{ number_format($b->harga_jual,0,',','.') }}</td>
                    <td class="py-4 px-6 text-center">
                        <span class="inline-flex items-center justify-center min-w-[3rem] px-2 py-1 rounded-lg {{ $b->stok < 10 ? 'bg-rose-100 text-rose-700' : 'bg-emerald-100 text-emerald-700' }} font-extrabold shadow-sm border {{ $b->stok < 10 ? 'border-rose-200' : 'border-emerald-200' }}">
                            {{ $b->stok }}
                        </span>
                    </td>
                    <td class="py-4 px-6 text-right">
                        <div class="flex items-center justify-end space-x-2">
                            <a href="{{ route('barang.edit', $b->id) }}" class="w-8 h-8 rounded-xl bg-slate-100 text-slate-500 hover:bg-indigo-100 hover:text-indigo-600 flex items-center justify-center transition-colors shadow-sm">
                                <i class="ph-bold ph-pencil-simple"></i>
                            </a>
                            <form action="{{ route('barang.destroy', $b->id) }}" method="POST" class="inline-block" onsubmit="return confirm('Yakin ingin menghapus barang ini?');">
                                @csrf
                                @method('DELETE')
