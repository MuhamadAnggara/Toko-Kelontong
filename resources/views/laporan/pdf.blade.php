...
<body onload="window.print()" class="antialiased max-w-5xl mx-auto">
    ...
    <div class="text-center mb-8 border-b-2 border-slate-800 pb-6">
        ...
        <p class="text-sm font-bold text-slate-400 mt-3">Dicetak pada: {{ date('d F Y - H:i') }} WIB</p>
    </div>
    ...
    <table class="w-full text-left border-collapse border border-slate-300">
        ...
        <tbody class="text-sm">
            ...
            <tr class="border-b border-slate-200">
                <td class="py-3 px-4 border border-slate-300 text-center font-bold">{{ $index + 1 }}</td>
                <td class="py-3 px-4 border border-slate-300 font-medium">
                    {{ $t->created_at->format('d M Y') }}<br>
                    <span class="text-xs text-slate-500">{{ $t->no_nota }}</span>
                </td>
                <td class="py-3 px-4 border border-slate-300 font-medium">
                    Kasir: {{ $t->user->name ?? 'Kasir M' }}<br>
                    <span class="text-xs {{ $t->metode_pembayaran == 'Tunai' ? 'text-indigo-600' : 'text-emerald-600' }} font-bold">{{ $t->metode_pembayaran }}</span>
                </td>
                <td class="py-3 px-4 border border-slate-300">
                    <ul class="list-disc pl-4 text-xs space-y-1">
                    @foreach($t->details as $d)
                        <li><b>{{ $d->barang->nama_barang ?? 'Barang Terhapus' }}</b> ({{ $d->qty }} item) - Rp{{ number_format($d->subtotal, 0, ',', '.') }}</li>
                    @endforeach
                    </ul>
                </td>
