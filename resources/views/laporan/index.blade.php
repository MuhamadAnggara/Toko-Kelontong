...
@section('header', 'Laporan Penjualan Toko')
...
<div class="bg-white rounded-3xl p-8 shadow-[0_8px_30px_rgb(0,0,0,0.04)] border border-slate-100 flex flex-col h-full overflow-hidden fade-in">
    <div class="flex justify-between items-center mb-6">
        ...
        <div class="flex space-x-3">
            <a href="{{ route('laporan.excel') }}" class="px-5 py-2.5 bg-emerald-50 text-emerald-600 font-bold rounded-xl border border-emerald-100/50 hover:bg-emerald-100 transition shadow-sm flex items-center">
                <i class="ph-bold ph-microsoft-excel-logo mr-2 text-lg"></i> Export Excel
            </a>
            <a href="{{ route('laporan.pdf') }}" target="_blank" class="px-5 py-2.5 bg-slate-900 text-white font-bold rounded-xl shadow-md shadow-slate-900/20 hover:bg-slate-800 transition flex items-center">
            ...
            <table class="w-full text-left border-collapse">
                ...
                <tbody class="divide-y divide-slate-100">
                    ...
                    <tr class="hover:bg-indigo-50/30 transition-colors group cursor-pointer group-hover">
                        <td class="py-5 px-6">
                            <span class="font-bold text-slate-800">{{ $t->created_at->format('d M Y') }}</span>
                            <span class="text-xs text-slate-400 block mt-0.5">{{ $t->created_at->format('H:i') }} WIB</span>
                        </td>
                        <td class="py-5 px-6">
                            <span class="px-3 py-1 bg-slate-100 text-slate-600 font-bold text-xs rounded-lg border border-slate-200">{{ $t->no_nota }}</span>
                        </td>
                        <td class="py-5 px-6">
                            <div class="flex items-center">
                                ...
                                <span class="font-semibold text-slate-700">{{ $t->user->name }}</span>
                            </div>
                        </td>
                        <td class="py-5 px-6">
                            <span class="px-3 py-1 font-bold text-xs rounded-lg {{ $t->metode_pembayaran == 'Tunai' ? 'bg-indigo-50 text-indigo-600 border border-indigo-100' : 'bg-emerald-50 text-emerald-600 border border-emerald-100' }}">
                                {{ $t->metode_pembayaran }}
                            </span>
                        </td>
                        <td class="py-5 px-6 text-right">
                            <span class="font-black text-slate-800">Rp {{ number_format($t->total_harga, 0, ',', '.') }}</span>
                        </td>
                        <td class="py-5 px-6 text-center">
                            <a href="{{ route('cetak.struk', $t->id) }}" target="_blank" class="inline-flex w-8 h-8 items-center justify-center rounded-lg bg-slate-100 text-slate-500 hover:bg-white hover:text-indigo-600 hover:shadow-md transition-all border border-transparent hover:border-indigo-100 tooltip cursor-pointer" title="Cetak Ulang Struk">
