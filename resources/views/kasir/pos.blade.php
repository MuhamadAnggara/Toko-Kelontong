...
@section('header', 'Mesin Kasir (Point of Sales)')
    ...
    <div class="flex-1 bg-white rounded-3xl p-6 shadow-[0_8px_30px_rgb(0,0,0,0.04)] border border-slate-100 flex flex-col h-full overflow-hidden">
        ...
        <div class="flex-1 overflow-y-auto pr-2 custom-scrollbar" id="katalog-produk">
            <div class="grid grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
                ...
                <div class="product-item bg-white border border-slate-200 rounded-2xl p-4 cursor-pointer hover:border-indigo-400 hover:shadow-lg transition-all transform hover:-translate-y-1 group" data-id="{{ $b->id }}" data-nama="{{ $b->nama_barang }}" data-harga="{{ $b->harga_jual }}" data-kode="{{ $b->kode_barang }}" data-stok="{{ $b->stok }}" data-gambar="{{ $b->gambar ? asset('storage/' . $b->gambar) : '' }}">
                    <div class="aspect-square bg-slate-50 rounded-xl mb-3 flex items-center justify-center overflow-hidden p-2">
                        @if($b->gambar)
                            <img src="{{ asset('storage/' . $b->gambar) }}" class="w-full h-full object-contain group-hover:scale-105 transition-transform" alt="{{ $b->nama_barang }}">
                        @else
                            <i class="ph-fill ph-package text-4xl text-slate-300"></i>
                        @endif
                    </div>
                    <p class="text-[10px] uppercase font-bold tracking-wider text-slate-400 mb-1">{{ $b->kode_barang }}</p>
                    <h3 class="font-extrabold text-slate-800 text-sm mb-2 line-clamp-2 leading-tight item-nama">{{ $b->nama_barang }}</h3>
                    <div class="flex justify-between items-end mt-auto">
                        <p class="font-bold text-indigo-600 text-sm">Rp{{ number_format($b->harga_jual, 0, ',', '.') }}</p>
                        <p class="text-xs font-semibold px-2 py-0.5 rounded-md {{ $b->stok < 10 ? 'bg-rose-100 text-rose-600' : 'bg-slate-100 text-slate-500' }}">Sisa {{ $b->stok }}</p>
                    </div>
                </div>
                @endforeach
            </div>
