...
<div class="bg-white rounded-3xl p-8 shadow-[0_8px_30px_rgb(0,0,0,0.04)] border border-slate-100">
    <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-8 pb-4 border-b border-slate-100">
        ...
        <a href="{{ route('kasir.create') }}" class="mt-4 sm:mt-0 flex items-center px-5 py-2.5 bg-gradient-to-r from-sky-500 to-blue-600 text-white rounded-xl shadow-lg shadow-sky-200 hover:shadow-sky-300 transform hover:-translate-y-0.5 transition-all text-sm font-bold">
    ...
    <div class="overflow-x-auto rounded-2xl border border-slate-200/60 shadow-sm">
        <table class="w-full text-left border-collapse">
            ...
            <tbody class="divide-y divide-slate-100 text-sm text-slate-700 font-medium">
                @forelse($kasirs ?? [] as $ksr)
                <tr class="hover:bg-blue-50/30 transition-colors group">
                    <td class="py-4 px-6 text-blue-600 font-bold font-mono">USR-{{ str_pad($ksr->id, 4, '0', STR_PAD_LEFT) }}</td>
                    <td class="py-4 px-6">
                        <div class="flex items-center">
                            <img src="https://ui-avatars.com/api/?name={{ urlencode($ksr->name) }}&background=e0f2fe&color=0369a1&rounded=true&bold=true" class="w-10 h-10 shadow-sm mr-3 rounded-full" alt="">
                            <span class="font-bold text-slate-800">{{ $ksr->name }}</span>
                        </div>
                    </td>
                    <td class="py-4 px-6">
                    ...
                    <td class="py-4 px-6 text-right">
                         <div class="flex items-center justify-end space-x-2">
                            <a href="{{ route('kasir.edit', $ksr->id) }}" class="w-8 h-8 rounded-xl bg-slate-100 text-slate-500 hover:bg-sky-100 hover:text-sky-600 flex items-center justify-center transition-colors shadow-sm">
                                <i class="ph-bold ph-pencil-simple"></i>
                            </a>
                            <form action="{{ route('kasir.destroy', $ksr->id) }}" method="POST" class="inline-block" onsubmit="return confirm('Yakin ingin menghapus kasir ini?');">
                                @csrf
                                @method('DELETE')
