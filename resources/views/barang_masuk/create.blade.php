...
@section('header', 'Input Transaksi Barang Masuk')
...
<div class="max-w-2xl mx-auto mt-6">
    <div class="bg-white rounded-3xl p-8 shadow-[0_8px_30px_rgb(0,0,0,0.04)] border border-slate-100 flex flex-col h-full overflow-hidden fade-in">
        ...
        <div class="flex items-center mb-8 border-b border-slate-200/60 pb-5">
            <a href="{{ route('barang-masuk.index') }}" class="w-10 h-10 bg-slate-100 text-slate-500 rounded-xl flex items-center justify-center hover:bg-slate-200 hover:text-slate-700 transition mr-4">
        ...
        <form action="{{ route('barang-masuk.store') }}" method="POST">
                    ...
                    <div class="relative">
                        ...
                        <select name="barang_id" required class="block w-full pl-11 pr-4 py-3 bg-slate-50 border border-slate-200 text-slate-800 rounded-xl focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 cursor-pointer appearance-none">
                            ...
                            @foreach($barangs as $b)
                                <option value="{{ $b->id }}">{{ $b->kode_barang }} - {{ $b->nama_barang }} (Sisa Stok: {{ $b->stok }})</option>
                            @endforeach
                        </select>
                ...
                <div class="grid grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        <div class="relative">
                            ...
                            <input type="date" name="tanggal" required value="{{ date('Y-m-d') }}" class="block w-full pl-11 pr-4 py-3 bg-slate-50 border border-slate-200 text-slate-800 rounded-xl focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500">
                        </div>
                    </div>
                </div>
            ...
            <div class="mt-8 pt-6 border-t border-slate-100 flex justify-end space-x-3">
                <a href="{{ route('barang-masuk.index') }}" class="px-6 py-3 bg-white text-slate-600 font-bold rounded-xl border border-slate-300 hover:bg-slate-50 transition">
                    Batal
                </a>
