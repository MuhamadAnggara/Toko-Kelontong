...
@section('header', 'Manajemen Karyawan (Admin & Kasir)')
                    ...
                    <tr class="hover:bg-indigo-50/30 transition-colors group">
                        <td class="py-4 px-6">
                            <div class="flex items-center">
                                <div class="w-10 h-10 rounded-full {{ $k->role == 'admin' ? 'bg-amber-100 text-amber-600' : 'bg-indigo-100 text-indigo-600' }} flex items-center justify-center font-bold mr-4 shadow-inner text-sm flex-shrink-0">
                                    {{ strtoupper(substr($k->name, 0, 1)) }}
                                </div>
                                <div>
                                    <h4 class="font-bold text-slate-800 text-sm">{{ $k->name }}</h4>
                                    <p class="text-xs font-mono text-slate-500 mt-0.5 bg-slate-100 px-1.5 py-0.5 rounded w-fit">{{ $k->username }}</p>
                                </div>
                            </div>
                        </td>
                        <td class="py-4 px-6">
                        ...
                        <td class="py-4 px-6">
                            <span class="font-bold text-slate-700 text-sm">{{ $k->created_at->format('d M Y') }}</span>
                            <span class="text-xs font-medium text-slate-400 block mt-0.5">{{ $k->created_at->diffForHumans() }}</span>
                        </td>
                        <td class="py-4 px-6 text-center">
                            <div class="flex items-center justify-center space-x-2">
                                <button onclick="openEditModal({{ $k->id }}, '{{ addslashes($k->name) }}', '{{ addslashes($k->username) }}', '{{ $k->role }}')" class="w-8 h-8 rounded-lg bg-slate-100 text-slate-500 hover:bg-emerald-50 hover:text-emerald-600 hover:border-emerald-200 transition border border-transparent flex items-center justify-center tooltip" title="Edit Akun Karyawan">
                                    <i class="ph-bold ph-pencil-simple text-sm"></i>
                                </button>
                                <form action="{{ route('owner.karyawan.destroy', $k->id) }}" method="POST" onsubmit="return confirm('Peringatan: Menghapus akun ini bersifat permanen. Apabila dihapus, user ini tidak bisa mengakses sistem lagi. Yakin hapus?');" class="inline">
                                    @csrf
                                    @method('DELETE')
...
<div id="modalTambah" class="fixed inset-0 bg-slate-900/80 backdrop-blur-sm z-50 hidden items-center justify-center p-4">
    <div class="bg-white rounded-3xl p-6 md:p-8 max-w-md w-full shadow-2xl relative transform transition-all">
        ...
        <form action="{{ route('owner.karyawan.store') }}" method="POST" class="space-y-4">
            @csrf
            <div>
...
<script>
    const baseUrl = "{{ url('owner/karyawan') }}";
