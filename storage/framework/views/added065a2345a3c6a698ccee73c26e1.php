<?php $__env->startSection('header', 'Edit Barang'); ?>
...
<div class="max-w-4xl mx-auto bg-white rounded-3xl p-8 shadow-[0_8px_30px_rgb(0,0,0,0.04)] border border-slate-100">
    <div class="mb-8 pb-4 border-b border-slate-100 flex items-center justify-between">
        <div>
            <h2 class="text-2xl font-extrabold text-slate-800">Edit Data Barang</h2>
            <p class="text-slate-500 text-sm mt-1">Sesuaikan harga, stok, atau detail barang</p>
        </div>
        <a href="<?php echo e(route('barang.index')); ?>" class="px-4 py-2 bg-slate-100 hover:bg-slate-200 text-slate-600 rounded-xl font-bold text-sm transition-colors flex items-center shadow-sm">
        ...
        <div class="space-y-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <div class="relative">
                        ...
                        <input type="text" name="kode_barang" id="kode_barang" value="<?php echo e(old('kode_barang', $barang->kode_barang)); ?>" required class="block w-full pl-11 pr-4 py-3 bg-slate-50 border border-slate-200 text-slate-800 rounded-xl focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 transition-colors placeholder-slate-400 font-mono shadow-inner font-medium uppercase">
                    </div>
                </div>
                    ...
                    <div class="relative">
                        ...
                        <select name="kategori_id" id="kategori_id" required class="block w-full pl-11 pr-4 py-3 bg-slate-50 border border-slate-200 text-slate-800 rounded-xl focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 transition-colors shadow-inner font-bold appearance-none">
                            ...
                            <?php $__currentLoopData = $kategoris; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $kategori): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                <option value="<?php echo e($kategori->id); ?>" <?php echo e(old('kategori_id', $barang->kategori_id) == $kategori->id ? 'selected' : ''); ?>><?php echo e($kategori->nama_kategori); ?></option>
                            <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                        </select>
            ...
            <div>
                ...
                <div class="relative">
                    ...
                    <input type="text" name="nama_barang" id="nama_barang" value="<?php echo e(old('nama_barang', $barang->nama_barang)); ?>" required class="block w-full pl-11 pr-4 py-3 bg-slate-50 border border-slate-200 text-slate-800 rounded-xl focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 transition-colors placeholder-slate-400 shadow-inner font-medium">
                </div>
            </div>
            ...
            <div>
                ...
                <div class="relative">
                    ...
                    <textarea name="deskripsi" id="deskripsi" rows="3" class="block w-full pl-11 pr-4 py-3 bg-slate-50 border border-slate-200 text-slate-800 rounded-xl focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 transition-colors placeholder-slate-400 shadow-inner font-medium" placeholder="Tuliskan informasi atau deskripsi detail tentang produk ini..."><?php echo e(old('deskripsi', $barang->deskripsi)); ?></textarea>
                </div>
            </div>

            <!-- Upload Gambar Produk (Baru ditambahkan) -->
            <div>
                <label for="gambar" class="block text-sm font-bold text-slate-700 mb-2 ml-1">Foto / Gambar Produk (Untuk Katalog) <span class="text-slate-400 font-normal text-xs ml-2">Opsional, biarkan kosong jika tidak diubah</span></label>
                <?php if($barang->gambar): ?>
                    <div class="mb-3">
                        <img src="<?php echo e(asset('storage/' . $barang->gambar)); ?>" alt="Preview" class="w-32 h-32 object-cover rounded-xl border border-slate-200">
                    </div>
                <?php endif; ?>
                <div class="relative">
                    <div class="absolute inset-y-0 left-0 pl-4 flex items-center pointer-events-none">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <div class="relative">
                        ...
                        <input type="text" name="harga_jual" id="harga_jual" value="<?php echo e(old('harga_jual', number_format($barang->harga_jual, 0, ',', '.'))); ?>" required class="block w-full pl-11 pr-4 py-3 bg-emerald-50 border border-emerald-100 text-emerald-800 rounded-xl focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500 transition-colors placeholder-emerald-300 font-bold shadow-inner" placeholder="0" oninput="this.value = this.value.replace(/[^0-9]/g, '').replace(/\B(?=(\d{3})+(?!\d))/g, '.');">
                    </div>
                </div>

                <!-- Stok -->
                <div>
                    <label for="stok" class="block text-sm font-bold text-slate-700 mb-2 ml-1">Stok Sisa <span class="text-rose-500">*</span></label>
                    <div class="relative">
                        <div class="absolute inset-y-0 left-0 pl-4 flex items-center pointer-events-none">
                            <i class="ph-fill ph-stack text-indigo-500 text-lg"></i>
                        </div>
                        <input type="number" name="stok" id="stok" value="<?php echo e(old('stok', $barang->stok)); ?>" required min="0" class="block w-full pl-11 pr-4 py-3 bg-slate-50 border border-slate-200 text-slate-800 rounded-xl focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 transition-colors placeholder-slate-400 shadow-inner font-bold">
                    </div>
                </div>
            </div>
...
<?php echo $__env->make('layouts.app', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH C:\xampp\htdocs\toko-kelontong\resources\views/barang/edit.blade.php ENDPATH**/ ?>